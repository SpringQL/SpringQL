...
//!
//! A TaskGraph has "virtual root stream", who has outgoing edges to all source streams.
//!
        ...
        let t_graph = p_graph.map(
            ...
            |_, edge| {
                let task = match edge {
                    ...
                    Edge::Source(source) => Task::Source(SourceTask::new(source)),
                    Edge::Sink(sink) => Task::Sink(SinkTask::new(sink)),
                };
                Arc::new(task)
            },
        );
