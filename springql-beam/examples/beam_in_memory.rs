...
use springql_beam::{SpringConfig, Pipeline, SpringSourceRowBuilder};

fn push_row_to_pipeline(pipeline: &Pipeline, queue_name: &str) {
    ...
    pipeline.apply(queue_name, row).unwrap()
}

fn main() {
    let pipeline = Pipeline::new(&SpringConfig::default()).unwrap();
